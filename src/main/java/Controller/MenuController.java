...
public class MenuController implements Subject
{
    ...
    private void loadDataMenu() {
        ...
        int choice = consoleView.requestInput("LOAD EMPLOYEE DATA","Select the CSV file to use from the resources folder\n" + consoleView.getLoadedDataStatus(), employeeFiles.toArray(new String[0]));
    ...
    private void runGeneticAlgMenu()
    {
        ...
        while(!exit)
        {
            ...
            switch (choice)
            {
                ...
                case 1:
                    GA_POPULATION_SIZE_DEFAULT = getParameter("Population size", GA_POPULATION_SIZE_DEFAULT, 1, Integer.MAX_VALUE);
                    break;
                case 2:
                ...
                case 4:
                    ...
                    break;
                case 5:
                     GA_REPORTING_FREQUENCY_DEFAULT = getParameter("Reporting frequency",GA_REPORTING_FREQUENCY_DEFAULT ,1, Integer.MAX_VALUE);
                    break;
                case 6:
    ...
    {
        int choice = (consoleView.requestInput("ENTER PARAMETER", "Enter a value for " +parameter.toLowerCase(), new String[]{"Exit", "True", "False"}));
        return (choice == 0) ? defaultVal : choice != 2;
    }

    private double getParameter(String parameter, double defaultVal,  double min, double max)
    {
        double choice = consoleView.requestInput("ENTER PARAMETER", "Enter a value for " + parameter.toLowerCase(), min, max);
        return ((choice == min-1.0) ? defaultVal : choice);
    }

    private int getParameter(String parameter, int defaultVal, int min, int max)
    {
        int choice = consoleView.requestInput("ENTER PARAMETER", "Enter a value for " + parameter.toLowerCase(), min, max);
        return ((choice == min-1.0) ? defaultVal : choice);
    }
